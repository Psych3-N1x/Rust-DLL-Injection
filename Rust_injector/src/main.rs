...
fn main() {
    let pid = if let Some(pid) = is_notepad_running() {
        println!("notepad.exe is already running with PID: {}", pid);
        pid
    } else {
        println!("notepad.exe is not running, launching it...");
        if let Some(pid) = launch_notepad_and_get_pid() {
            println!("Successfully launched notepad.exe with PID: {}", pid);
            pid
        } else {
            println!("Failed to launch notepad.exe or get its PID");
            return;
        }
    };
    ...
    let path_to_dll = "C:\\Users\\chmod\\Documents\\dll.dll\0"; // enter the correct path to your DLL 
...
fn is_notepad_running() -> Option<u32> {
    find_process_pid("notepad.exe")
}

fn launch_notepad_and_get_pid() -> Option<u32> {
    ...
    match command.spawn() {
        ...
        Err(e) => {
            println!("Failed to launch notepad.exe: {}", e);
            None
        }
    }
}
